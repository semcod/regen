...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Harp - Musical string manipulation tool"
    )
    parser.add_argument(
        "--generate", "-g", action="store_true", help="Generate audio from harp"
    )
    parser.add_argument(
        "--notes",
        "-n",
    ...
    parser.add_argument(
        "--duration", "-d", type=float, default=2.0, help="Duration in seconds"
    )
    parser.add_argument(
        "--output", "-o", default="harp_output.wav", help="Output audio file"
    )
    parser.add_argument(
        "--strings", "-s", type=int, default=47, help="Number of strings on the harp"
    )
    parser.add_argument(
